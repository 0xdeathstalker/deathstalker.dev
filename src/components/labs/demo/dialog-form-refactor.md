...
- **Submit button pending spinner** — `DialogFormSubmit` swaps its label for a spinner (default lucide `Loader`, overridable via `spinner` prop) while status is "submitting", using an internal keyed `AnimatePresence mode="popLayout"` slide (enter from above, exit below — Emil's button-states pattern). Keyed by rendered content, not raw status, so error → idle doesn't replay the swap. Styled via `buttonVariants` (`variant`/`size` props, same surface as `ui/button.tsx`); width should be fixed by the consumer (e.g. `w-20`) since the label and spinner differ in width. `relative overflow-hidden` are baked defaults the animation depends on. The nested presence is safe: no layoutIds inside (same rule as DialogFormView).
  **Constraint discovered:** `layout` on a node inside the panel gives it a separate entry animation during the open morph, visibly detaching it from the content (observed with `layout` on the submit button; since removed). Don't put `layout`/`layoutId` on panel children unless they're part of the shared-element choreography.
