...
export function ScrollToTop({ threshold = 400 }: { threshold?: number } = {}) {
  ...
  return (
    <button
      ...
      className={cn(
        "fixed bottom-6 right-6 z-50 p-2 border border-dashed border-muted-foreground/45 bg-background/80 backdrop-blur-sm text-black hover:text-foreground hover:opacity-100 transition-all duration-300 cursor-pointer",
        visible
