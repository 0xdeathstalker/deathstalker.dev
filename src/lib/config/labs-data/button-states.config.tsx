...
const buttonStatesConfig = {
  ...
  description: (
    <>
      <p>
        this is an exploration into animating button states with pure css. in an interview i was asked: how do you animate
        an exit transition with pure css in a react application? the problem is that react removes elements from the dom
        immediately on unmount, without waiting for any animation to finish. this is the same limitation that{" "}
        <Code>AnimatePresence</Code> from motion solves but, here the goal was to achieve it without any library.
      </p>
      <p>
        the button in the above example has four states: <Code>idle</Code>, <Code>loading</Code>, <Code>success</Code> and{" "}
        <Code>failure</Code>. instead of conditionally rendering each state, all four are kept in the dom at all times.
      ...
      <div>
        ...
        <ul className="my-1">
          ...
          <li className="flex items-center w-full leading-snug">
            <div className="pr-2 self-start text-muted-foreground/40">•</div>
            <span>
              the button size stays stable as the idle <Code>span</Code> holds the layout while other states layer on top
              absolutely
            </span>
          </li>
        </ul>
      ...
      <p className="mb-4">
        overall, this is a lightweight technique well-suited for simple, predictable state transitions. it is also a good
        fit when bundle size is a constraint, for example, in performance-critical apps where avoiding an animation
